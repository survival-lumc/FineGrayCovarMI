...
df_summ_var[stat == "Coverage", stat_lab := paste0(
  "bold('", stat, " %:')~'",
  format(round(est * 100, 1), nsmall = 1), 
  "'~('", 
  format(round(mcse * 100, 1), nsmall = 1), 
  "')"
)]
df_summ_var[, lab_y := ifelse(prob_space == 0.15, 0.26, 0.15)]
...
for (i in seq_along(covars)) {
  ...
  lims <- if (covars[i] == "X") c(0.05, 0.3) else c(0, 0.2)
...
# Find the intercept again
intercept_shift <- uniroot(
  f = function(eta_0, linpred, prob) mean(plogis(eta_0 + linpred)) - prob,
  interval = c(-25, 25),
  extendInt = "yes",
  linpred = with(dat_missmech, 1.5 * Z),
  prob =  0.4
)$`root`

dat_missmech[, prob_miss_X := plogis(1.5 * Z + intercept_shift)]
...
dat_missmech <- generate_dataset(
  ...
  args_missingness = list(
    mech_params = list("prob_missing" = 0.4, "mechanism_expr" = "-1.5 * log(time + 1)")
  )
)
...
intercept_shift <- uniroot(
  ...
  interval = c(-25, 25),
  extendInt = "yes",
  linpred = with(dat_missmech, -1.5 * log(time + 1)),
  prob =  0.4
)$`root`

dat_missmech[, prob_miss_X := plogis(-1.5 * log(time + 1) + intercept_shift)]

# Potentially on log scale, let's see
p_jitt <- dat_missmech |> 
  ggplot(aes(time, X_obs, col = prob_miss_X)) +
  geom_jitter(aes(shape = factor(X_missind)), height = 0.25, alpha = 0.35, size = 2.5) +
  scale_color_viridis_c(direction = -1) +
  scale_shape_manual("X missing", values = c(16, 17), labels = c("No", "Yes")) +
  labs(y = "X", col = "Probability\nX missing", x = "Observed competing event time") +
  coord_cartesian(xlim = c(0, 10))

p_logis <- dat_missmech |> 
  ggplot(aes(time, prob_miss_X)) +
  geom_line(linewidth = 2, col = cols[6]) +
  labs(y = "Probability of X missing", x = "Observed competing event time") +
...
dat_sub |> 
  ggplot(aes(V_true, log(H1_subdist_true / Lambda_T))) +
  geom_point(col = "#2F3D70", alpha = 0.75, size = 3) +
  coord_cartesian(xlim = c(0, 10)) +
  labs(
    x = expression(italic(V)),
    y = expression(
      #log~'{'~hat(Lambda)[1](italic(V))/hat(Lambda)[1](italic(T))~'}'
      hat(Lambda)[1](italic(V))/hat(Lambda)[1](italic(T))
    ) 
  ) +
  scale_y_continuous(
    breaks = log(c(1, 2, 4, 8, 16, 32, 64, 128)),
...
mar_t_coefs[, relbias := 100 * (estimate - true) / true]
...
```{r}
#| label: rel-bias-plots-covarcens
#| fig-height: 7
#| out-width: '80%'
...
covar_cens_coefs[, ':=' (
  ...
  rhs_kmi = factor(
    rhs_kmi, 
    levels = c("1", "Z"), 
    labels = c("Marginal (incorrect)", "Stratif. by Z (correct)")
  )
)]
...
df |>
  ...
  stat_summary(
    data = df[rhs_kmi == "Stratif. by Z (correct)"],
    geom = "segment",
    fun = function(x) mean(x) - crit * (sd(x) / sqrt(length(x))),
    fun.max = function(x) mean(x) + crit * (sd(x) / sqrt(length(x))),
    aes(xend = method, yend = after_stat(ymax), col = method),
