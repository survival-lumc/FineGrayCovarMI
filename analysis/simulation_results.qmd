...
# Checks
mod_marg <- coxph(Surv(V_true, D == 1) ~ 1, data = dat)
mod <- update(mod_marg, . ~ X + Z)
base_marg <- basehaz(mod_marg, centered = FALSE)
base <- basehaz(mod, centered = FALSE)

const <- base_marg$hazard / base$hazard
with(base_marg, plot(time, hazard, type = "s"))
with(base, lines(time, hazard, type = "s", col = "blue"))
with(base, lines(time, hazard * const, type = "s", col = "red", lwd = 2))
...
# Get max true one, extrapolate back
max_subdist_marg <- max(dat$H1_subdist_true)
t_max_subdist <- dat[order(V_true)][H1_subdist_true == max_subdist_marg, .SD[1]]$V_true
max_cuminc_marg <- 1 - exp(-max_subdist_marg) # used as p

# Approx bit
breaks_time <- pmin(t_max_subdist, c(0.1, 1.25, 2.5, 3.75, 5, 7.5, 10))

cuminc_approx <- max_cuminc_marg *
  (1 - exp(-params$cause1$base_rate * breaks_time^params$cause1$base_shape))
subdist_approx <- -log(1 - cuminc_approx)
breaks_sec_axis <- round(subdist_approx, 3) #as.numeric(floor(subdist_approx * 100) / 100)

p_ids <- merge(imp_dats_cens, dat)[id %in% id_samps] |> 
  melt(
  ...
  scale_y_continuous(
    sec.axis = sec_axis(
      trans = ~ {
        cuminc <- max_cuminc_marg * (1 - exp(-params$cause1$base_rate * .^params$cause1$base_shape))
        -log(1 - cuminc)
      }, 
      breaks = breaks_sec_axis, 
      name = "Cumulative subdistribution hazard at timepoint"
    )
  )

#plot(dat$V_true, dat$H1_subdist_true)
#points(
#  dat$V_true,
#  -log(1 - (max_cuminc_marg * (1 - exp(-1 * dat$V_true^0.75)))),
#  col = "blue"
#)


# Regular plot
p_subdists <- imp_dats_cens |>
  ggplot(aes(newtimes, H1_subdist)) +
  geom_step(aes(group = .imp), alpha = 0.75, col = "gray") +
  theme(legend.position = "none") +
  coord_cartesian(xlim = c(0, 10), ylim = c(0, 1.75)) + #, ylim = c(0, 2)) +
