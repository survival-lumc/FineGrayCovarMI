...
breaks_sec_axis <- round(f(breaks_time), 3) 
...
p_ids <- merge(imp_dats_cens, dat)[id %in% id_samps] |> 
  ...
  geom_point(aes(col = time_var, shape = time_var, size = time_var), alpha = 0.5) +
  scale_size_manual(
    "Timepoint",
    values = c(3, 1.5, 3),
...
ggsave(
  plot = p_ids / p_subdists,
  "analysis/figures/cens_appendix.pdf", 
  dpi = 300,
  width = 9,
  height = 10,
  device = cairo_pdf
