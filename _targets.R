...
source(here("R/one-replication.R"))
# or
...
dgm_params <- tar_map(
  values = list("p" = scenarios$prob_space_domin[!duplicated(scenarios$prob_space_domin)]),
  tar_target(
...
sim_fun <- tar_map_rep(
  name = all_sims,
  values = scenarios, # What we are iterating over
  command = {
    eval(parse(text = paste0("true_params_correct_FG_", prob_space_domin)))
  },
  reps = 2,
  batches = 1,
  combine = TRUE#,
  #names = c("failure_time_model", "censoring_type")
)
