...
run_imp_methods <- function(obj,
  ...
  # Method x: MICE with cumulative subdistribution hazard
  predmat_subdist <- predmat
  predmat_subdist[] <- 0
  predmat_subdist["X", c("Z", "newevent", "H_subdist_cause1")] <- 1

  # Method x: SMC-FCS with cause-specific hazards
  ...
  # Arguments specific to certain methods
  ...
  mod_summaries <- if (cens_times_known) {
    ...
    nested_impdats <- rbind(
      data.table(
        method = "full",
        mods = list(
          lapply(obj, function(impdat) model_fun(model_formula_full, data = impdat))
        )
      ),
      data.table(
        method = "CCA",
        mods = list(
          lapply(obj, function(impdat) model_fun(model_formula, data = impdat))
        )
      )
    )
